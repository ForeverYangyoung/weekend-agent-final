...
from backend.constraints_util import blocked_poi_ids, candidate_passes_constraints
from backend.schemas import ConstraintTracker, GroupProfile, POICandidate, Plan
...
def f_health(c: POICandidate, *, constraints: ConstraintTracker | None = None) -> float:
    cal = c.metadata.get("avg_calories_per_meal")
    if cal is not None and constraints is not None:
        budget = constraints.remaining_calories
        if float(cal) <= budget * 0.6:
            return 1.0
        if float(cal) > budget:
            return 0.1
...
def compensator_score(
    ...
    *,
    ...
    constraints: ConstraintTracker | None = None,
) -> float:
    """Score(p) = α·f_health + β·f_child - γ·D"""
    scene = profile.scene if profile else "unknown"
    health = f_health(c, constraints=constraints)
    child = f_child(c)
    if scene == "friends":
        health = 0.5
...
def pick_best_alternative(
    ...
    *,
    ...
    constraints: ConstraintTracker | None = None,
    anomaly_encountered: list[str] | None = None,
) -> POICandidate | None:
    reserved = set(exclude_poi_ids or ())
    reserved.add(failed_poi_id)
    reserved |= blocked_poi_ids(anomaly_encountered)
    ...
    pool = [
        c
        for c in pool
        if c.poi_id not in reserved
        and candidate_passes_constraints(c, constraints, stage_name=stage_name)
    ]
    if not pool:
        pool = _pool_for_stage(
            candidates,
    ...
    ranked = sorted(
        pool,
        key=lambda c: compensator_score(
            c, anchor_km=anchor, profile=profile, constraints=constraints
        ),
        reverse=True,
    )
    return ranked[0]

...
def _score_distance_math(c: POICandidate, profile: GroupProfile) -> float:
    """Sigmoid 平滑距离惩罚，替代生硬截断。"""
    d = float(c.metadata.get("distance_km", 0) or 0)
    limit = max(profile.distance_limit_km, 1.0)
    k = 2.0
    return 1.0 / (1.0 + math.exp(k * (d - limit)))


def _score_budget_math(
    ...
    return math.exp(-5.0 * over_ratio)
...
def _score_one(
    ...
    dist = _score_distance_math(c, profile)
    budget = _score_budget_math(c, profile, stage_name)
    total = (

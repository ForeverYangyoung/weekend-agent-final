...
from backend.schemas import ConstraintTracker, FailureType, Plan, PlanStage, ToolStatus
...
def _failed_poi_from_state(state: AgentState) -> str:
    current = state.get("current_plan") or {}
    if current.get("failed_poi_id"):
        return str(current["failed_poi_id"])
    seats = _failed_seat_calls(state)
    if seats:
        return seats[0][1]
    return "unknown"


def _apply_full_seat_rollback_state(
    state: AgentState,
    failed_poi_id: str,
) -> dict:
    """满座自愈：拉黑商户 + 修正儿童疲劳度，闭环不抛错。"""
    base = state.get("constraints") or build_constraints(state.get("group_profile"))
    constraints: ConstraintTracker = apply_full_seat_constraint_update(
        base, failed_poi_id=failed_poi_id
    )
    anomalies = list(state.get("anomaly_encountered") or [])
    marker = f"{failed_poi_id}_full"
    ...
    current_plan = {
        ...
        "blocked_pois": sorted(blocked_poi_ids(anomalies)),
    }
    return {
        "constraints": constraints,
        "anomaly_encountered": anomalies,
        "current_plan": current_plan,
    }


def _refresh_plan_summary(plan: Plan, profile) -> Plan:
    from backend.agents.planner import _summary

    order_label = plan.order_label or " → ".join(s.name for s in plan.stages)
    scene = profile.scene if profile else "unknown"
    return plan.model_copy(
        update={"summary": _summary(scene, plan.stages, order_label)}
    )
...
def execute_poi_substitution(state: AgentState) -> dict:
    ...
    for stage_name, failed_pid in failures:
        ...
        sc = compensator_score(
            best, anchor_km=anchor, profile=profile, constraints=constraints
        )
        old_name = stage.primary.name
        rollback = _apply_full_seat_rollback_state(state, failed_pid)
        constraints = rollback["constraints"]
        anomalies = rollback["anomaly_encountered"]
        ...
        patched = True
        fatigue = constraints.child_fatigue_index if constraints else 0
        traces.append(
            trace_line(
                "Executor",
                f"NO_SEAT·{stage_name}｜{old_name} → {best.name}｜Score={sc:.2f}｜"
                f"拉黑 {failed_pid} 疲劳度→{fatigue}",
                phase="恢复",
            )
        )
    ...
    healed_alts: list[Plan] = []
    for alt in state.get("plan_alternatives") or []:
        alt_copy = alt.model_copy(deep=True)
        for stage_name, failed_pid in failures:
            stage = next((s for s in alt_copy.stages if s.name == stage_name), None)
            if stage is None or stage.primary.poi_id != failed_pid:
                continue
            pool = _candidates_for_stage(state, stage_name)
            alt_best = pick_best_alternative(
                pool,
                failed_poi_id=failed_pid,
                plan=alt_copy,
                stage_name=stage_name,
        ...
        alt_copy = _rebind_addons(alt_copy, state)
        alt_copy = _refresh_plan_summary(alt_copy, profile)
        alt_copy.score = _plan_score_math(alt_copy)
        healed_alts.append(alt_copy)

    updated = _rebind_addons(updated, state)
    updated = _refresh_plan_summary(updated, profile)
    ...
    last_failed = failures[-1][1] if failures else _failed_poi_from_state(state)
    rollback = _apply_full_seat_rollback_state({**state, "constraints": constraints}, last_failed)
    rollback["current_plan"]["failed_poi_id"] = None
    rollback["current_plan"]["summary"] = updated.summary
    traces.append(
        trace_line(
            "Executor",
            f"自愈闭环：商户 {last_failed} 已拉黑，儿童疲劳度="
            f"{rollback['constraints'].child_fatigue_index}",
            phase="恢复",
        )
    )
    return {
        "plan": updated,
        "plan_alternatives": healed_alts,
        ...
        "constraints": rollback["constraints"],
        "anomaly_encountered": rollback["anomaly_encountered"],
        "current_plan": rollback["current_plan"],
        "trace": traces,
    }
